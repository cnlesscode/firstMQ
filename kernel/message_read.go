...
func ReadMessages(
	...
	startIndex,
	readLength int64) ([]MessageForRead, int64, error) {
	// 消息区间不能大于分片容量
	...
	// 1. 获取索引
	dataLogPaths, messageOffsetMap, err := GetMessageIndexData(
		topicName,
		startIndex,
		messageCountNumber)
	if err != nil {
...
func GetMessageIndexData(
	...
	startFileIndex := startIndex / configs.FirstMQConfig.FragmentCapacity

	// 检查消息是否位于2个分片中
	endIndex := startIndex + length
	endFileIndex := endIndex / configs.FirstMQConfig.FragmentCapacity

	// 同一个分片
	if startFileIndex == endFileIndex {
		logFilePath, indexFilePath := InitLogFiles(topicName, startFileIndex)
		...
		startIndexFor := startIndex - startFileIndex*configs.FirstMQConfig.FragmentCapacity
	...
	endLengthForFirst := configs.FirstMQConfig.FragmentCapacity*(startFileIndex+1) - startIndex
	startForFirst := startIndex - startFileIndex*configs.FirstMQConfig.FragmentCapacity
	var step1 int64 = 0
	for step1 = 0; step1 < endLengthForFirst; step1++ {
		indexFile.Seek((step1+startForFirst)*16, 0)
		var offsetStart int64 = 0
		binary.Read(indexFile, binary.LittleEndian, &offsetStart)
		var Length int64 = 0
		binary.Read(indexFile, binary.LittleEndian, &Length)
	...
	// 再读取第2个文件
	logFilePath, indexFilePath = InitLogFiles(topicName, endFileIndex)
	dataLogPaths = append(dataLogPaths, logFilePath)
	...
	endLengthForSecond := endIndex - configs.FirstMQConfig.FragmentCapacity*endFileIndex
	var step2 int64 = 0
	for step2 = 0; step2 < endLengthForSecond; step2++ {
		indexFile2.Seek(step2*16, 0)
		var offsetStart int64 = 0
